...
- `Lmayu == -1`：強制停止。
- リミットスイッチ`lock7`（`reset_flag`）検知で自動的に排出/リセット動作（スピン逆転）を開始し、`set_flag`を立てて2000msのあいだ逆転させたのち停止する自動シーケンスあり。
...
| 変数 | 説明 |
|---|---|
| `m1〜m4` | ジョイスティックから計算したオムニミックス値（前左/前右/後左/後右）、最大速度制限のため×0.7 |
| `PV1〜PV6` | CAN受信データ（`use_data`）から変換した現在値（各モーターの実測フィードバック）。PV1〜4は駆動輪、PV5・PV6はローラーモーター用 |
| `pwm1〜pwm8` | 実際にPWM出力するduty値（TIM4=駆動輪、TIM1=ローラー/付属モーター）。`motor_control`の積分器そのもの |
| `dir1〜dir4` | 駆動モーター4基分の回転方向GPIO状態。ローラー（pwm5/pwm6）は常に正転で方向転換しないため共用の捨て変数`dummy`を渡す |
| `rem1〜rem8` | `motor_control`の積分計算で切り捨てられる端数の繰り越し（各モーターごとに保持、`\|rem\| < 10`） |
...
| 変数 | 説明 |
|---|---|
| `stop_flag, reset_flag` | リミットスイッチ`lock6`/`lock7`の状態。ローラーの停止・排出シーケンス制御に使用 |
| `set_flag` | 排出シーケンス実行中フラグ。`reset_flag`検知で立ち、2000ms経過で降りる |
| `roller_dir` | ローラー回転アクチュエータの回転方向 |
| `timer_flag`, `time3` | リミットスイッチ検知後の800ms待機タイマー制御用 |
| `time4` | 排出シーケンス（スピン逆転2000ms）の開始時刻 |
...
| 関数 | ファイル | 説明 |
|---|---|---|
| `motor_control(SV, PV, maxMV, down_pwm, max_pwm, *pwmm, *dirr, *remm)` | function.c | 汎用モーター制御。目標値と現在値の差を積分してPWMを制御し（積分器は`pwm`自身）、上限速度・変化率制限、方向反転時の減速シーケンスを行う。`remm`は整数除算の端数キャリーで、`\|error\| < 10`の不感帯を解消する。全8モーターで共通利用 |
...
| `sbus(void)` | sbus_handler.c | SBUS入力をスイッチ/スティック値に変換し、オムニミックス`m1〜m4`を計算 |
